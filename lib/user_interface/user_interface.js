...
class UserInterface extends EventEmitter {
    ...
    addYoutubePlaylist(name, url) {
        this.storage.Playlist.findOrCreateByName(name).then((playlist) => {
            YouTubeDl.getInfo(url, ['--flat-playlist'], (err, results) => {
                results.forEach((info) => {
                    this.Track.create({
                        ...
                        this.commandConsole.dearUser(track.title + ' added');
                    }).catch((err) => {
                        this.commandConsole.dearUser(err.message);
                    });
                });
