...
class UserInterface extends EventEmitter {
    ...
    events() {
        ...
        this.commandConsole.on('command_open', () => {
            let playlist = this.playlist.getSelected();
            if (this.playlist.isLoaded() && playlist) {
                this.log.log('Loading ' + playlist.title);
                YouTubeDl.getInfo(playlist.url, (err, info) => {
                    let audioStream = info.formats
                                          .filter(({vcodec}) => vcodec == 'none')
        ...
        this.player.player.on('time', () => {
            this.player.container.setContent(this.player.player.status.raw);
            this.screen.render();
        });
    }
