...
var defaultArgs = ['-nolirc', '-msglevel', 'global=6', '-msglevel', 'cplayer=4', '-slave', '-idle'];
...
class Player extends EventEmitter {
    ...
    spawn(args = []) {
        ...
        instance.stderr.on('data', this.onError);
    ...
    onError(data) {
        ...
        console.log('Mplayer experienced an error');
        process.exit(1);
    }
    ...
    parseStdout(input) {
        ...
        if(input.indexOf('A:') === 0) {
            ...
            this.emit('timechange', time, total);
            return true;
        }
