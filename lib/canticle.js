...
class Canticle extends EventEmitter {
    ...
    currentTrack() {
        return this.loadedPlaylistTracks[this.loadedTrack];
    }
    ...
    openPlaylist(playlist) {
        if (playlist.hasOwnProperty('failure') && playlist.failure) {
            ...
            playlist.tracks.forEach((track) => {
                this.playlistList.addItem(track.title);
                this.loadedPlaylistTracks.push(track);
            });
            this.log.log(playlist.name + ' was loaded.');
            if (this.loadedPlaylistTracks.length) {
                this.emit('start_playback', this.currentTrack());
            }
        }
        this.screen.render();
    }
