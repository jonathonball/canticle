...
class Canticle extends EventEmitter {
	...
	playlistManagerLayout() {
		...
		this.playlistManagerList.addItem('test');
		this.playlistManagerList.addItem('test2');
	}

	playlistManagerEvents() {
		this.playlistManagerConsole.on('submit', (userInput) => {
			this.screen.log('user entered: ' + userInput);
			this.playlistManagerLog.log(userInput);
			this.emit('playlistManagerConsole', userInput);
